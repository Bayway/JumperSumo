...
public abstract class SuffixTree {
	...
	private static void test (String string) {
		...
		System.out.println("'turn' occurs at " + toString(tree.findAll("turn")));
		System.out.println("'left' occurs at " + toString(tree.findAll("left")));
		System.out.println();
	}
	...
	public static void main (String[] args) {
		...
		// test("hi$my$friend$can$you$go$on$and$turn$left$");
		test("turn$left$and$go$on$and$turn$left$");
		...
		SuffixTree tree;
		tree = new MySuffixTree("turn$left$and$go$on$and$turn$left$");
		for (int i=0; i<key.length; i++)
		{
			int[] trovato = tree.findAll(key[i]);
			if (trovato.length > 0)
			{
				map.put(key[i], trovato);
			}
		}
		Integer[] consec = f.checkConsecutive(key, map);
		System.out.println("");
		System.out.println("Elementi consecutivi: ");

		for (int i : f.toPrimitive(consec))
